...
function performMigration() {
  ...
  APP_VERSION_CLEAN=$(echo "${APP_VERSION}" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
  ...
  if [ "${APP_VERSION_CLEAN}" == "${LAST_VERSION_MIGRATED}" ]; then
      ...
      for dir in ${GZL_MIGRATION_VERSION_FOLDER}/* ; do # Do not add quotes here or you will loose 1 hour
          ...
          if [ "$(echo "${VERSION}" | { IFS=. read a b c; echo $((10#$a*10000+10#$b*100+10#$c)); })" -gt "$(echo "${LAST_VERSION_MIGRATED}" | { IFS=. read a b c; echo $((10#$a*10000+10#$b*100+10#$c)); })" ] \
            && [ "$(echo "${VERSION}" | { IFS=. read a b c; echo $((10#$a*10000+10#$b*100+10#$c)); })" -le "$(echo "${VERSION}" | { IFS=. read a b c; echo $((10#$a*10000+10#$b*100+10#$c)); })" ]; then
