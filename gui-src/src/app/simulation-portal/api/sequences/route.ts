...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${process.env.GZL_SIMULATION_GATEWAY_URL}${queryString ? "?" + queryString : ""}`;
    ...
    console.error("Error fetching simulation sequences:", error);
    return NextResponse.json({ error: "Failed to fetch simulation sequences" }, { status: 500 });
  }
}
