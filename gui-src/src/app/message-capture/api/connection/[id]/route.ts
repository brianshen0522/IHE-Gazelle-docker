...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = new URL(`${process.env.GZL_DTH_API_URL}/items/${id}${readAccessKeyQueryParam}`);
    ...
    return NextResponse.json({ error: err?.message || "Unable to get connection by id" }, { status: err.response?.status || 500 });
  }
}
