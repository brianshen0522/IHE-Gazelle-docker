...
export async function GET(req: NextRequest) {
  try {
    ...
    const config: AxiosRequestConfig | undefined = {responseType: "stream"};
    if (token) {
      config.headers = {
    ...
    return NextResponse.json({ error: err?.message || "Unable to get attachment", status: err.response?.status || 500 });
  }
}
