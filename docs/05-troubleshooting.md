...
| Keycloak shows an "HTTPS required" error page on login (plain-http deployment reached from another machine) | realms default to `sslRequired=EXTERNAL`, rejecting http from non-private client IPs | rerun `./scripts/apply-public-url.sh` (with `PUBLIC_SCHEME=http` it sets `sslRequired=NONE` on both realms); manual: `kcadm update realms/<master|gazelle> -s sslRequired=NONE` |
...
| Home page redirects to install/installation.seam; the wizard then redirects to `/login/ipLogin.seam` → 404 | the TM DB has no organization, so TM launches its legacy install wizard, whose login page (ipLogin, non-SSO) is deliberately not routed by nginx | never use the wizard in this setup — run `apply-public-url.sh` + `bootstrap-admin.sh`; the wizard disappears once an organization exists. If bootstrap was already run → see the MQTT row above |
